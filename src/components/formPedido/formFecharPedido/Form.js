...
const Form = ({ getUsers, onEdit, setOnEdit }) => {
    ...
    if (onEdit) {
      await axios
        .put("http://localhost:8800/" + onEdit.id, {
          nome: user.pedido.value,
          email: user.cliente.value,
          fone: user.produto.value,
          data_nascimento: user.quantidade.value,
        })
  ...
  const handleFecharPedido = async (e) => {
    ...
    const requestBody = JSON.stringify(
      ...
      fetch("http://localhost:3000/projects/fechar-pedido", {
        method: 'POST',
        headers: headers,
        body: requestBody,
      })
        ...
        .then(data => {
          // Faça algo com 'data' conforme necessário
        })
        .catch(error => {
          console.log(error)
    ...
    const getPedido = async (selectedValue) => {
      ...
      try {
        ...
        setPedido(res.data.sort((a, b) => (a.nome > b.nome ? 1 : -1)));
      } catch (error) {
        toast.error(error);
      }
      console.log(pedido);
